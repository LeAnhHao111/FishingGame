...
// Check va cham
bool Level4::CheckCollision(std::shared_ptr<SpriteAnimation> sprite1, std::shared_ptr<Fish> sprite2)
{
...
void Level4::CheckHookCollision()
{
...
void Level4::Init()
{
	...
	auto texture = ResourceManagers::GetInstance()->GetTexture("bg_sea4.jpg");
	...
	buttonnext->SetOnClick([this]() {
		GameStateMachine::GetInstance()->ChangeState(StateType::LEVEL5);
		});
	...
	auto fish0 = std::make_shared<Fish>(CHEAT);
	texture = ResourceManagers::GetInstance()->GetTexture("chest.png");
	fish0->m_fishAnimation = std::make_shared<SpriteAnimation>(texture, 1, 3, 1, 0.5f);
	fish0->SetSize(80, 80);
	fish0->Set2DPosition(920, 390);
	m_fish.push_back(fish0);

	fish0 = std::make_shared<Fish>(CHEAT);
	texture = ResourceManagers::GetInstance()->GetTexture("chest.png");
	fish0->m_fishAnimation = std::make_shared<SpriteAnimation>(texture, 1, 3, 1, 0.5f);
	fish0->SetSize(80, 80);
	fish0->Set2DPosition(255, 415);
	m_fish.push_back(fish0);
	...
	for (int i = 1; i <= 15; i++) {
		...
		case 4:
		{
			auto fish0 = std::make_shared<Fish>(Fish6);
			texture = ResourceManagers::GetInstance()->GetTexture("fish7.png");
			fish0->m_fishAnimation = std::make_shared<SpriteAnimation>(texture, 1, 2, 1, 0.2f);
			fish0->SetSize(45, 18);
			fish0->Set2DPosition(m_pos.x, m_pos.y);
			m_fish.push_back(fish0);
		...
		case 5:
		{
			auto fish0 = std::make_shared<Fish>(Fish7);
			texture = ResourceManagers::GetInstance()->GetTexture("fish8.png");
			fish0->m_fishAnimation = std::make_shared<SpriteAnimation>(texture, 1, 2, 1, 0.2f);
			fish0->SetSize(45, 18);
			fish0->Set2DPosition(m_pos.x, m_pos.y);
			fish0->m_fishAnimation->SetFlip(SDL_FLIP_HORIZONTAL);
		...
		case 6:
		{
			auto fish0 = std::make_shared<Fish>(Fish5);
			texture = ResourceManagers::GetInstance()->GetTexture("fish6.png");
			fish0->m_fishAnimation = std::make_shared<SpriteAnimation>(texture, 1, 2, 1, 0.2f);
			fish0->SetSize(108, 44);
			fish0->Set2DPosition(m_pos.x, m_pos.y);
			fish0->m_fishAnimation->SetFlip(SDL_FLIP_HORIZONTAL);
			m_fish.push_back(fish0);
			break;
		...
		switch (i%4) 
		{
			...
			case 2:
			{
				auto fish0 = std::make_shared<Fish>(TRASH2);
				texture = ResourceManagers::GetInstance()->GetTexture("trash2.png");
				fish0->m_fishAnimation = std::make_shared<SpriteAnimation>(texture, 1, 4, 1, 0.2f);
				fish0->SetSize(60, 60);
				fish0->Set2DPosition(m_pos.x, m_pos.y);
				fish0->m_fishAnimation->SetFlip(SDL_FLIP_HORIZONTAL);
			...
			case 3:
			{
				auto fish0 = std::make_shared<Fish>(TRASH4);
				texture = ResourceManagers::GetInstance()->GetTexture("trash4.png");
				fish0->m_fishAnimation = std::make_shared<SpriteAnimation>(texture, 1, 4, 1, 0.2f);
				fish0->SetSize(80, 80);
				fish0->Set2DPosition(m_pos.x, m_pos.y);
				fish0->m_fishAnimation->SetFlip(SDL_FLIP_HORIZONTAL);
	...
	m_currentPlayerState = PlayerState4::IDLE4;
...
void Level4::HandleEvents()
{
}

void Level4::HandleKeyEvents(SDL_Event& e)
{
...
void Level4::HandleTouchEvents(SDL_Event& e)
{
...
void Level4::HandleMouseMoveEvents(int x, int y)
{
}
...
void Level4::Update(float deltaTime)
{
	...
	{
	case 1: // left 
		if (m_currentPlayerState & (PlayerState4::IDLE4 | MOVE_RIGHT4))
		{
			m_currentPlayerState = PlayerState4::MOVE_LEFT4;
		}
		break;
	case 1 << 1: // down
		if (m_currentPlayerState & (PlayerState4::IDLE4 | PlayerState4::MOVE_LEFT4 | PlayerState4::MOVE_RIGHT4))
		{
			m_currentPlayerState = PlayerState4::CAST4;
		}
		break;
	case 1 << 2: // right
		if (m_currentPlayerState & (PlayerState4::IDLE4 | MOVE_LEFT4))
		{
			m_currentPlayerState = PlayerState4::MOVE_RIGHT4;
		}
		break;
	case 1 << 3: // up
	...
	if (!(m_KeyPress & 0b101) && // none of left and right are pressed
		!(m_currentPlayerState & (PlayerState4::CAST4 | REEL4))) // not casting or reeling
	{
		m_currentPlayerState = PlayerState4::IDLE4;
	}
	...
	case PlayerState4::CAST4:
		...
		{
			m_currentPlayerState = PlayerState4::REEL4;
			break;
		}
	...
	if (m_score >= 1500 && !m_isMissionCompleted)
	{
