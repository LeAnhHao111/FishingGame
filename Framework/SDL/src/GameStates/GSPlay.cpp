...
// Check va cham
bool GSPlay::CheckCollision(std::shared_ptr<SpriteAnimation> sprite1, std::shared_ptr<Fish> sprite2)
{
...
void GSPlay::CheckHookCollision()
{
...
void GSPlay::Init()
{
	...
	auto texture = ResourceManagers::GetInstance()->GetTexture("bg_sea1.jpg");
	...
	buttonnext->SetOnClick([this]() {
		GameStateMachine::GetInstance()->PopState();
		GameStateMachine::GetInstance()->PushState(StateType::LEVEL2);
		});
...
void GSPlay::Update(float deltaTime)
{
	...
	{
	case 1: // left 
		if (m_currentPlayerState & (PlayerState::IDLE | MOVE_RIGHT))
		{
			m_currentPlayerState = PlayerState::MOVE_LEFT;
		}
		break;
	case 1 << 1: // down
		if (m_currentPlayerState & (PlayerState::IDLE | PlayerState::MOVE_LEFT | PlayerState::MOVE_RIGHT))
		{
			m_currentPlayerState = PlayerState::CAST;
		}
		break;
	case 1 << 2: // right
		if (m_currentPlayerState & (PlayerState::IDLE | MOVE_LEFT))
		{
	...
	if (!(m_KeyPress & 0b101) && // none of left and right are pressed
		!(m_currentPlayerState & (PlayerState::CAST | REEL))) // not casting or reeling
	{
		m_currentPlayerState = PlayerState::IDLE;
	}
	...
	case PlayerState::CAST:
		...
		{
			m_currentPlayerState = PlayerState::REEL;
			break;
		}
	...
	if (m_score >= 520 && !m_isMissionCompleted)
	{
